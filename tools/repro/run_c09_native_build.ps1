# Configure and build C09 in a fresh, stage-specific native Windows tree.
[CmdletBinding()]
param(
...
$sourceStatusBeforeBuild = @(& git -C $source status --porcelain=v1 -uall)
$sourceDirtyBeforeBuild = $sourceStatusBeforeBuild.Count -gt 0
if ($sourceDirtyBeforeBuild) {
    throw 'C09 final build requires a clean implementation commit before evidence capture.'
}
...
if (-not $build.StartsWith($allowedParent, [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw "C09 build directory must be a distinct path below $allowedParent; got $build"
}
...
Set-Content -LiteralPath $captureLog -Encoding utf8 -Value @(
    'C09_NATIVE_BUILD_LOG_V1'
    "source=$source"
    "build=$build"
    "source_head=$sourceHeadBeforeBuild"
    "source_tree=$sourceTreeBeforeBuild"
    'source_dirty=false'
)
...
function Invoke-CapturedPowerShell(
    ...
    if ($exitCode -ne 0) { throw "C09 $Label failed: $exitCode" }
}
...
$geometryContract = Join-Path $source 'contracts\p11\GEOMETRY_CONTRACT_V1.json'
$decisionContract = Join-Path $source 'contracts\decision_reconciliation.json'
$record = [ordered]@{
    artifact_type = 'C09_NATIVE_BUILD'
    stage = 'C09/P11'
    status = 'PASS'
    source = [ordered]@{
        path = $source
    ...
    contracts = [ordered]@{
        geometry_sha256 = (Get-FileHash -LiteralPath $geometryContract -Algorithm SHA256).Hash.ToLower()
        decision_reconciliation_sha256 = (Get-FileHash -LiteralPath $decisionContract -Algorithm SHA256).Hash.ToLower()
    }
    ...
    warnings = @(
        'Inherited MSVC C4244 instantiations originate in the pinned AMReX/std::numeric code path; no C09 source warning was emitted.'
    )
...
Write-Host "C09 native build evidence: $json"
