# Configure and build C11 from a clean commit in a fresh native Windows tree.
[CmdletBinding()]
param(
...
if ($sourceStatus.Count -gt 0) {
    throw 'C11 final build requires a clean implementation commit.'
}
...
if (-not $build.StartsWith(
    ...
    throw "C11 build directory must be distinct and below $allowedParent; got $build"
}
...
function Invoke-CapturedPowerShell(
    ...
    if ($exitCode -ne 0) { throw "C11 $Label failed: $exitCode" }
}
...
$contract = Join-Path $source 'contracts\p13\REACTION_LIEBIG_CONTRACT_V1.json'
$fit = Join-Path $source 'contracts\p13\OPERATOR_FIT_V1.json'
$order = Join-Path $source 'contracts\p10\OPERATOR_ORDER_V1.json'
$authority = Join-Path $source 'contracts\USER_APPROVED_DECISION_CONTRACT_20260801.txt'
$record = [ordered]@{
    artifact_type = 'C11_NATIVE_BUILD'
    stage = 'C11/P13'
    status = 'PASS'
    source = [ordered]@{
        path = $source
    ...
    contracts = [ordered]@{
        reaction_liebig_sha256 = (Get-FileHash -LiteralPath $contract -Algorithm SHA256).Hash.ToLower()
        ...
        authority_source_sha256 = (Get-FileHash -LiteralPath $authority -Algorithm SHA256).Hash.ToLower()
    }
    ...
    warnings = @(
        'Two inherited MSVC C4244 diagnostics originate in std::numeric instantiated by pinned AMReX particle I/O; no C11 source warning was emitted.'
    )
    external_resources = 'none'
    external_spend_usd = 0
    claim_boundary = 'Clean Windows native C11 engineering build; not release-host, mentor-approved, calibrated, predictive, or C12+ evidence.'
}
...
Write-Host "C11 native build evidence: $json"
