...
> I adopt the attached contract as the binding user-approved decision contract, superseding the pending mentor gate for P10-U01. I authorize Codex as the sole C08 integrator to modify src/des/bmx_calc_txfr.cpp, src/timestepping/bmx_evolve.cpp, src/bmx.H, and src/bmx.cpp only as needed to complete C08. This does not authorize C09 or later work.

Additional exact C08 source-ownership authorization:

> I authorize Codex during C08 only to modify src/des/bmx_pc.H and src/des/bmx_pc.cpp solely to wrap inherited Redistribute and Regrid overloads with the existing preflight guard before delegating to pinned AMReX. This authorizes no wall projection/sliding and no C09+ behavior.
